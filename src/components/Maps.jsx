      ...
      <LeafletMap className="map" center={plantations[0]} zoom={2}>
        ...
        {plantations.map((plantation) => {
          function getCam() {
            axios
              .get(
                `https://api.windy.com/api/webcams/v2/list/category=forest/nearby=${plantation[0]},${plantation[1]},1000/limit=1?show=webcams:location,image,player&key=${APIKey}`
              )
            ...
            <Marker onClick={() => getCam()} position={plantation}>
              <Popup>
                <div className="spanCss">
                  ...
                  <p>
                    trizz to plant:{" "}
                    {trizz - plantedTrizz > 0 ? trizz - plantedTrizz : 0}
                    <br />
                    Are you sure to plant here ?
                  </p>
                  ...
                  <Modal
                    ...
                    header={Cam.map((cam) => {
                      return <p key={cam.id}>{cam.title}</p>;
                    })}
                    content={
                      Cam[0] !== undefined ? (
                        Cam.map((cam) => {
