...
import "./Home.css";
import "./test.css";
import { Grid, Segment, Popup, Item, Image } from "semantic-ui-react";
        ...
        <Grid.Row>
          ...
          <Grid.Column width={4}>
            <Segment id="previewArticle">
              ...
              {articles
                .filter((article) => articles.indexOf(article) < 3)
                .map((article) => {
                  return (
                    <Item key={article.id}>
                      <Item.Content href={article.url} target="_blanck">
                        <Item.Header className="titleArticle" as="a">
                          {article.title}
                        </Item.Header>
                        <Item.Description>
                          {article.image === "None" ? (
